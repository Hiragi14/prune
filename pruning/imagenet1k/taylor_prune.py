...
def get_imagenet1k_dataloaders(
    ...
    Args:
        ...
        data_dir (str, optional): データディレクトリのパス。 Defaults to "/workspace/dataset/ImageNet/".
...
def taylor_prune(
    device: str = typer.Option(
        "cuda" if torch.cuda.is_available() else "cpu", "-d", "--device", help="Device to use"
    ),
    iterative_steps: int = typer.Option(
        5, "-i", "--iter", help="Number of iterative pruning steps"
    ),
    density: float = typer.Option(0.6, "-r", "--density", help="Target density after pruning"),
    epochs: int = typer.Option(1, "-e", "--epochs", help="Number of fine-tuning epochs"),
):
    ...
    train_loader, test_loader = get_imagenet1k_dataloaders(
        batch_size=256,
        device=device_,
        train_trsfm=imagenet1k_train_trsfm(),
        valid_trsfm=imagenet1k_valid_trsfm(),
    )
    ...
    for iter in range(iterative_steps):
        ...
        for k, (images, labels) in track(
            enumerate(train_loader), description="Calculating importance scores...", transient=True
        ):
...
def taylor_prune_loop(
    ...
    for density in densities:
        report = taylor_prune(
            device=device, iterative_steps=iterative_steps, density=density, epochs=epochs
        )
