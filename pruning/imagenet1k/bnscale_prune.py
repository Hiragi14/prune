...
def bnscale_prune(
    ...
    torch.save(
        ...
        f"pruned_bnscale_resnet18_{density:.2f}_{iterative_steps}_iter_ft{epochs}epoch.pth",
    )
    save_prune_summary(
        report,
        path=f"prune_report_bnscale_{density:.2f}_{iterative_steps}_iter_ft{epochs}epoch.json",
    )
    return report
